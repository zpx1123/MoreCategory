...
+ (NSString *)dataWithTimeUnix{
    
    NSDate* newData = [NSDate dateWithTimeIntervalSinceNow:0];
    NSTimeInterval a = [newData timeIntervalSince1970] * 1000;
...
+ (NSString *)convertDateIntervalToStringWith:(NSString *)aInterVal
{
    ...
    else if (distance < (60 * 60 * (thour))) /* 大于1小时，但是在今日 */
    {
        ...
        if(mins<10){
            timestamp =[NSString stringWithFormat:@"%@ %d:0%d",@"今天",ptime->tm_hour ,ptime->tm_min];
        }
        else
        {
            timestamp =[NSString stringWithFormat:@"%@ %d:%d",@"今天",ptime->tm_hour ,ptime->tm_min];
        }
    }
...
+(NSString *)DateIntervalToStringWith:(NSInteger )aInterVal{
    time_t    statusCreateAt_t;
    NSString* timestamp = nil;
    time_t now;
    time(&now);
    
    statusCreateAt_t = (time_t)aInterVal;
    ...
    double distance = (double)difftime(now, statusCreateAt_t);
    
    if (distance < 0) distance = 0;
    
    
    if (distance < (60 * 60 * (thour))) /* 大于1小时，但是在今日 */
    {
        int mins=ptime->tm_min;
        if(mins<10){
            timestamp =[NSString stringWithFormat:@"%@ %d:0%d",@"今天",ptime->tm_hour ,ptime->tm_min];
    ...
    else if (distance >= (60 * 60 * thour) && distance < (60 * 60 * 24 * 31))
    {
        ...
        if (distance <2)
        {
            NSDate *nd = [NSDate dateWithTimeIntervalSince1970:statusCreateAt_t];
            
            NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
            [dateFormat setDateFormat:@"HH:mm"];
            NSString *dateString = [dateFormat stringFromDate:nd];
            timestamp = [NSString stringWithFormat:@"%@ %@",@"昨天",dateString];
        }
        else if (distance >= 2)
        {
            NSDate *nd = [NSDate dateWithTimeIntervalSince1970:statusCreateAt_t];
