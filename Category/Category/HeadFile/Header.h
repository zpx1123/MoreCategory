...
#define	XLogOut(format,...);      //(format, ##__VA_ARGS__);
#define XLogOutMethodFun          //( @"[%@] %@", NSStringFromClass([self class]), NSStringFromSelector(_cmd) );
#define XLogError(format,...);    //(@"[error][%s][%d]" format,__func__,__LINE__,##__VA_ARGS__);
#define XLogWaring(format,...);   //(@"[waring][%s][%d]" format,__func__,__LINE__,##__VA_ARGS__);
